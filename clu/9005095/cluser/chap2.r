...
V-function, we can view the state set S as a subset of
.nf
			[Dv1  Rv1] \x ... \x [Dvn  Rvn] = 9D*
.fi
where {v1,...,vn} is the set of non-derived and hidden V-functions of the 
module.  In most cases, S is a proper subset of 9D*.  This occurs when an n-tuple
...
of the module.  Hence, we view the state set, S, of a Parnas module
in the following manner:

	S  [Dv1  Rv1] \x ... \x [Dvn  Rvn] = 9D*
...
description.
	Our purpose in this section is to define S.  Here, a generative approach 
...
.ls 1
.keep
		1) Q is an element of S.

		2) If q is an element of S and o is an O-function call,
		then the state q' obtained by applying o to q is an element of S.

		3) These are the only members of S.
.end_keep
.ls 2

So, to define S, it suffices to define the initial state of the module and
...
(initv1,...,initvn) containing the mapping derived from the
...
.ls 1
	...
	2)   If R8*S and o is an O-function, then if NEXT(R,o(a)) is
defined,
	NEXT(R,o(a))8*S where a8*Do.

	3)   These are the only elements of S. 
.end_keep
.ls 2

	In other words, the state set S is the closure of Q under the state
...
.keep
	Definition
	A Parnas module 2well-defined* if for any s8*S and O-function o
...
S \x Do into the Booleans and 9T*o is a total function from
{(s,a)8*S \x Do | 9A*o(s,a)} into S.  This can be seen by inspection of the
definition of NEXT.
.subsection "The Semantics of V-functions and O-functions"
	With this definition of the state set S of a Parnas module
...
O-Eval returns a function from Do into S  {error}.
...
and derivation derv,
		...
		V-Eval(s,v) = @a.[9A*v(s,a)dervs(a),error]
...
.keep
	Definition
	A Parnas module is 2consistent* if V-Eval(s,v) is a total function from Dv
into Rv  {error}
	for every state s8*S and V-function v of PM.
.end_keep
.ls 2


	In a consistent Parnas module, vs is a total function from
...
S \x Do into the Booleans and 9T*o is a total function from
{(s,a)8*S \x Do | 9A*o(s,a)} into S.
	Since S is defined generatively, a state machine
...
is defined for all ai8*Doi.
...
v and state s8*S, vs is a total function from
...
reach a state and input symbol for which its next state fucntion is undefined.  So, in
