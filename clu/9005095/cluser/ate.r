...
.ls 2
	DEFINE, 'MULTIMETER', SENSOR, AC SIGNAL, FREQ MAX 140 HZ, VOLTAGE MAX 15V,
.ls 1
		CURRENT MAX 1 MA, UUT-IMP 40+J50 OHM, CNX HI J1-2 LO J1-3$
.ls 2
.br
...
a given time frame which is insufficent for all tasks to be executed.
...
the UUT.  ATLAS supports block structure in its grouping of procedural statements.
	Conditional statements in ATLAS are of the form
		IF boolean expresion THEN statements END,IF
.br
or
...
conditional depending on a time limit, test result, comparision or manual intervention.
...
complex functions of a number of variables.
	In ALTAS, contact with the UUT is accomplished through signal oriented
...
such as DELAY.  These statemennts are listed in Figure 3.
.begin_figure "ATLAS Signal Oriented Statements"
.sp 3
	Single Action
...
.finish_figure
	Associated with the noun and noun modifiers are units of measure.
Contact with the UUT requires the language allow the specification of units of
measure.  There are several reasons for the inclusion of units of measure into an
ATE language.  One is the improvement they lend to the readability of a program.
Another use of units of measure in an ATE language is to provide a method of
double checking the type of stimulus which the programmer is applying to the UUT.
...
.subsubsection "Discussion"
	An ATLAS program, taken from [Ellison 7], is given below in Figure 7.
...
function designators are specified in a data bank associated which contains the
information necessary to perfrom the appropriate hardware linkage.
...
grouped together in a table.  Ceratin of the GOAL command and response statements can
...
listed in that table recieving the state in their row.  Function designators
...
primitives for the types of operations most compilers perform [Gries 68].
The principal aids provided by existing compiler generators are scanner generaters,
parser generators and code gneration facilities.
.subsection "UTEC"
