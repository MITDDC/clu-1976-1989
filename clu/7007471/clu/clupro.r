...
.end_list
As soon as step (2) has been performed, new problems exist
concerning how to implement the abstractions defined in step (2).
The programmer can choose to work on one of these problems
immediately, before steps (3) and (4) have been carried out
...
abstractions that are used.
	To make effective use of this methodology,
...
wordtree = cluster is
	...
	node = record [value: string, count: int, lesser: wordtree, greater: wordtree];
...
.table
	sorted_bag = cluster [t: type] is create, insert, ... 
		where t has less_than, equal: proc [t, t] returns [bool];
.end_table
...
sorted_bag = cluster [t: type] is create, insert, size, increasing;
		where t has equal, less_than: proc [t, t] returns [bool];
...
increasing = iter (sb: cvt) yields (t, int);
	for item: t, count: int in tree[t]$increasing (sb) do
		yield item, count;
		end;
	end increasing;
...
tree = cluster [t: type] is create, insert, increasing;
	...
	node = record [value: t, count: int, lesser: tree[t], greater: tree[t]];
	rep = oneof [empty: null, non_empty: node];
...
increasing = iter (x: cvt) yields (t, int);
	tagcase x
		tag empty: ;
		tag non_empty (n: node):
			for item: t, count: int in tree[t]$increasing (x.lesser) do
				yield item, count;
				end;
			yield x.value, x.count;
			for item: t, count: int in tree[t]$increasing (x.greater) do
				yield item, count;
				end;
		end;
	end increasing;
...
it uses an iterator
.table
	next_word = iter (i: instream) yields (string);
		...
.end_table
